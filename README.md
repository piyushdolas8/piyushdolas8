<p align="center">
  <img src="Banner1.png" alt="GitHub Banner" width="100%">
</p>

<h1 align="center">Hi 👋, I'm Piyush Dolas</h1>

<h3 align="center">
AI Developer • Full-Stack Developer • Embedded Systems
</h3>

<p align="center">
Building intelligent software and embedded systems that solve real-world problems.
</p>

<p align="center">
🎓 Electronics & Computer Engineering Student • Pune, India
</p>

<p align="center">
<a href="https://www.linkedin.com/in/piyush-dolas-0567a6303">
LinkedIn
</a>
•
<a href="mailto:piyushdolas8@gmail.com">
Email
</a>
</p>

---

## 🚀 About Me

I'm an **Electronics & Computer Engineering student** passionate about building intelligent software and embedded systems that solve real-world problems.

My interests lie in:

- 🤖 Artificial Intelligence & Machine Learning
- 💻 Full-Stack Development
- 🔌 Embedded Systems & IoT
- ⚡ Automation & AI Workflows

I enjoy transforming ideas into practical products—from AI-powered applications and automation tools to IoT systems and modern web applications.

Currently, I'm focused on improving my skills in **Data Structures & Algorithms, System Design, Backend Development, and AI Engineering** while building projects that bridge software and hardware.

---

## 🛠 Tech Stack

### Languages
<p>
  <img src="https://skillicons.dev/icons?i=python,cpp,c,java,javascript,sql" />
</p>

### Frontend
<p>
  <img src="https://skillicons.dev/icons?i=react,html,css,tailwind" />
</p>

### Backend
<p>
  <img src="https://skillicons.dev/icons?i=fastapi,nodejs" />
</p>

### Databases
<p>
  <img src="https://skillicons.dev/icons?i=mysql,mongodb,supabase" />
</p>

### Tools & Platforms
<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,aws,vercel" />
</p>

### Embedded Systems
- ESP32
- Arduino
- IoT Sensors
- Proteus
  
  ---

---

## 🌱 Currently Working On

- 🌐 Building **CRUZO** — A goal-based networking platform for students.
- 🤖 Developing AI-powered applications using **Google Gemini** and **FastAPI**.
- 💻 Strengthening **Data Structures & Algorithms** with **C++**.
- 🔌 Exploring **Embedded Systems** and **IoT** development.
- 📚 Continuously learning through hands-on projects and open-source.

---

## 📚 Currently Learning

- Data Structures & Algorithms
- System Design Fundamentals
- Machine Learning
- FastAPI & Backend Development
- Embedded AI & IoT

---

## 🏆 Certifications

- 📜 **NPTEL** — Data Mining for Decision Making
- 🍃 **MongoDB** — Building AI-Powered Search with Vector Search
- ☁️ **AWS** — Introduction to Generative AI: Art of the Possible
- 💼 **Deloitte Forage** — Software Engineering & Technology Job Simulations

---

## 🎯 Goals for 2026

- 🚀 Launch the first version of **CRUZO**
- 🤖 Build impactful AI-powered applications
- 💻 Contribute to Open Source
- 📚 Strengthen DSA & System Design
- 🏆 Secure a Software / AI Engineering Internship

---

## 💡 Engineering Philosophy

> *"The best way to learn engineering is by building real-world solutions, sharing the journey, and continuously improving with every project."*

---

## 🤝 Connect With Me

<p align="left">
  <a href="https://www.linkedin.com/in/piyush-dolas-0567a6303" target="_blank">
    <img src="https://skillicons.dev/icons?i=linkedin" height="45" alt="LinkedIn"/>
  </a>
  &nbsp;
  <a href="mailto:piyushdolas8@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" height="45" alt="Email"/>
  </a>
  &nbsp;
  <a href="https://www.youtube.com/@piyushbuilds2026" target="_blank">
    <img src="https://cdn.simpleicons.org/youtube/FF0000" height="45" alt="YouTube"/>
  </a>
</p>
